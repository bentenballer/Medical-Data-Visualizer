...
df["overweight"].where(df["overweight"]>25, 0, inplace=True)
df["overweight"].where(df["overweight"]<=25, 1, inplace=True)
...
def draw_heat_map():
    ...
    sns.heatmap(corr,mask=mask,cmap=cmap,annot=True,fmt=".1f")
